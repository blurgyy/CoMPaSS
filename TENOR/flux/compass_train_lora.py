...
from image_datasets.scop import SCOPDataset
...
def main():
    ...
    train_dataset = SCOPDataset(
        root=args.data_config.root,
        prompt_dropout=args.data_config.prompt_dropout,
        image_size=args.data_config.img_size,
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(dit):
                img, prompts = batch["pixel_values"], batch["text"]
                with torch.no_grad():
                    ...
                    if accelerator.is_main_process:
                        ...
                        for i, prompt in enumerate(args.sample_prompts):
                            ...
                            save_dir = os.path.join(
                                args.output_dir, f"samples-{global_step}"
                            )
                            os.makedirs(save_dir, exist_ok=True)
                            save_path = os.path.join(save_dir, f"{i:02d}.png")
                            result.save(save_path)
                            print(
                                f"Result for prompt #{i} is generated and saved at {save_path}"
                            )
