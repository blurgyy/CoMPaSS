...
class XFluxPipeline:
    ...
    def set_lora(
        ...
        checkpoint = load_checkpoint(local_path, repo_id, name)
        self.update_model_with_lora(checkpoint, lora_weight, use_tenor)

    def set_lora_from_collection(
        self, lora_type: str = "realism", lora_weight: float = 1.0
    ):
    ...
    def update_model_with_lora(self, checkpoint, lora_weight, use_tenor: bool):
            ...
            if len(lora_state_dict):
                if name.startswith("single_blocks"):
                    ...
                    lora_attn_procs[name] = DoubleStreamBlockLoraProcessor(
                        dim=3072, use_tenor=use_tenor, rank=rank
                    )
