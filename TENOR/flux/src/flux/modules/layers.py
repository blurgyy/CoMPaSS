...
class DoubleStreamBlockLoraProcessor(nn.Module):
    def __init__(
        self, dim: int, use_tenor: bool, rank=4, network_alpha=None, lora_weight=1
    ):
        super().__init__()
    ...
    def forward(self, attn, img, txt, vec, pe, **attention_kwargs):
        ...
        if self.use_pe:
            ...
            txt_modulated_with_pe = txt_modulated + self.pe(txt_modulated)
            txt_qkv_with_pe = (
                attn.txt_attn.qkv(txt_modulated_with_pe)
                + self.qkv_lora2(txt_modulated_with_pe) * self.lora_weight
            )
            txt_q, txt_k, _ = rearrange(
                txt_qkv_with_pe, "B L (K H D) -> K B H L D", K=3, H=attn.num_heads
            )
