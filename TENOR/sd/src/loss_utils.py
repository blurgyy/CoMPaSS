...
def get_word_idx(text: str, tgt_word, tokenizer):
    ...
    # add 1 for sot token
    tgt_word_tokens_idx_ls = [
        i + 1 + first_token_idx for i in range(len(encoded_tgt_word))
    ]
...
def get_grounding_loss_by_layer(
    _gt_seg_list, word_token_idx_ls, res, input_attn_map_ls
):
    ...
    for attn_map in input_attn_map_ls:
        ...
        for i in range(
            len(word_token_idx_ls)
        ):  # [[word1 token_idx1, word1 token_idx2, ...], [word2 token_idx1, word2 token_idx2, ...]]
            obj_loss = 0.0
    ...
    for i in range(len(input_attn_map_ls)):
        avg_attn_map_ls.append(
            input_attn_map_ls[i]
            .reshape(-1, res, res, input_attn_map_ls[i].shape[-1])
            .mean(0)
        )
