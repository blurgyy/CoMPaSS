...
def main(args):
    train_layers_ls = (
        [f"down_{res}" for res in args.train_down]
        + [f"mid_{res}" for res in args.train_mid]
        + [f"up_{res}" for res in args.train_up]
    )
    ...
    with accelerator.main_process_first():
        logger.info(f"train data dir: {args.train_data_dir}")

        train_dataset = SCOPDataset(
    ...
    register_attention_control(unet, controller, args.pe_type)
    ...
    if accelerator.is_main_process:
        ...
        tracker_config.pop("train_up")
        tracker_config.pop("train_mid")
        tracker_config.pop("train_down")
        ...
        accelerator.init_trackers(
            project_name=args.tracker_project_name, config=tracker_config
        )
    ...
    logger.info(f"  Image resolution: {args.resolution}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if (
                    prompts[0] == ""
                ):  # prompt has been dropped, only use denoising loss here.
                    token_loss = pixel_loss = 0.0
                elif token_loss_scale <= 0.0 and pixel_loss_scale <= 0.0:
                    token_loss = pixel_loss = 0.0
                else:
                ...
                grounding_loss = (
                    token_loss_scale * token_loss + pixel_loss_scale * pixel_loss
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        help=("res 512 for sd14, res 768 for sd21"),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            ' (default), Use `"all"` to report to all integrations.'
        ),
    )
    ...
    parser.add_argument("--token_loss_scale", type=float, required=True)

    parser.add_argument(
        "--train_down",
        nargs="+",
        type=int,
        help="use which res layers in U-Net down",
        default=[],
    )
    parser.add_argument(
        "--train_mid",
        nargs="+",
        type=int,
        help="use which res layers in U-Net mid",
        default=[],
    )
    parser.add_argument(
        "--train_up",
        nargs="+",
        type=int,
        help="use which res layers in U-Net up",
        default=[],
    )
    ...
    parser.add_argument("--pe_type", choices=get_args(PeType) + (None,), default=None)
